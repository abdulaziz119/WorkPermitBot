...
export class RequestsService {
  ...
  async createRequest(
    workerId: number,
    reason: string,
    requestType: RequestTypeEnum = RequestTypeEnum.DAILY,
    approvedDate?: Date,
    returnDate?: Date,
    hourlyLeaveTime?: Date,
    hourlyRequestType?: HourlyRequestTypeEnum,
    ...
    const entity: RequestEntity = this.repo.create({
      ...
      hourly_leave_time: hourlyLeaveTime || null,
      hourly_request_type: hourlyRequestType || null,
  ...
  async listPendingDaily(): Promise<RequestEntity[]> {
    return this.listPendingByType(RequestTypeEnum.DAILY);
  }

  async listPendingHourly(): Promise<RequestEntity[]> {
    return this.listPendingByType(RequestTypeEnum.HOURLY);
  }
  ...
  async getApprovedLeaveForToday(
    workerIds: number[],
